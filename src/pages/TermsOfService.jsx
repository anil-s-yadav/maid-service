import { Header } from "@/components/Header";
import { Footer } from "@/components/Footer";
import { FileText } from "lucide-react";
import { BRAND } from "@/utils/constants";

const TermsOfService = () => {
  const lastUpdated = "July 2, 2026";

  return (
    <div className="min-h-screen dark:bg-[#0f172a] bg-slate-50 flex flex-col transition-colors duration-500">
      <Header />

      {/* Hero Section */}
      <section className="relative pt-32 pb-16 md:pt-40 md:pb-20 dark:bg-[#0a0f1e] bg-white overflow-hidden border-b dark:border-white/10 border-slate-200 transition-colors duration-500">
        <div className="absolute inset-0 z-0 pointer-events-none">
          <div className="absolute top-0 left-1/4 w-96 h-96 bg-brand-gold/10 rounded-full blur-[100px] -translate-y-1/2"></div>
          <div className="absolute bottom-0 right-1/4 w-80 h-80 bg-blue-500/5 rounded-full blur-[80px] translate-y-1/2"></div>
        </div>
        
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-brand-gold/10 text-brand-gold mb-6 border border-brand-gold/20 shadow-lg shadow-brand-gold/5">
              <FileText className="w-8 h-8" />
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold font-heading dark:text-white text-slate-900 mb-6 transition-colors">
              Terms of <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-gold to-amber-400">Service</span>
            </h1>
            <p className="text-lg md:text-xl dark:text-slate-400 text-slate-600 mb-4 transition-colors">
              The rules, guidelines, and agreements for using our services.
            </p>
            <div className="inline-block dark:bg-white/5 bg-slate-100 rounded-full px-4 py-1.5 dark:text-slate-400 text-slate-500 text-sm font-medium border dark:border-white/10 border-slate-200 transition-colors">
              Last Updated: {lastUpdated}
            </div>
          </div>
        </div>
      </section>

      {/* Content Section */}
      <section className="py-16 md:py-24 flex-grow relative z-20">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="dark:bg-[#1e293b] bg-white rounded-3xl p-8 md:p-12 lg:p-16 shadow-xl border dark:border-white/10 border-slate-200 transition-colors duration-500 prose prose-slate dark:prose-invert max-w-none prose-headings:font-heading prose-headings:font-bold prose-a:text-brand-gold hover:prose-a:text-amber-500 prose-img:rounded-xl">
            
            <p className="lead text-lg md:text-xl text-slate-600 dark:text-slate-300">
              Welcome to {BRAND.name}. These Terms of Service govern your use of our website and our domestic placement services in Mumbai. By accessing our website or engaging our services, you agree to be bound by these terms.
            </p>

            <hr className="my-8 border-slate-200 dark:border-white/10" />

            <h2>1. Service Agreement</h2>
            <p>
              {BRAND.name} acts as a placement agency facilitating the connection between clients seeking domestic help and candidates providing such services. We do not act as the employer of the candidates. The employment contract is strictly between the client (employer) and the candidate (employee).
            </p>

            <h2>2. Placement Fee and Replacements</h2>
            <ul>
              <li><strong>Placement Fee:</strong> A one-time non-refundable placement fee is charged upon the successful hiring of a candidate. The fee structure is detailed in our pricing section.</li>
              <li><strong>Replacement Guarantee:</strong> We offer a standard replacement guarantee of up to 3 replacements within the first 6 months of employment, should the candidate leave or be found unsuitable. This guarantee is valid only if the full placement fee has been paid on time.</li>
              <li><strong>Trial Period:</strong> We may offer a short paid trial period (e.g., 2-3 days) before final confirmation, subject to the candidate's agreement and service terms.</li>
            </ul>

            <h2>3. Client Responsibilities</h2>
            <p>As the employer, you agree to:</p>
            <ul>
              <li>Provide a safe, respectful, and legally compliant working environment for the candidate.</li>
              <li>Pay the agreed-upon salary directly to the candidate on time, as per your mutual agreement.</li>
              <li>Not hold {BRAND.name} legally or financially liable for any acts of misconduct, theft, or damage caused by the candidate, although we conduct rigorous police and background verifications prior to placement.</li>
            </ul>

            <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-6 my-8">
              <h3 className="text-blue-500 mt-0 mb-3 font-bold">Important Notice Regarding Verification</h3>
              <p className="text-sm dark:text-slate-300 text-slate-700 m-0">
                While we ensure 100% Aadhaar verification and initiate police background checks for all our candidates, clients are strongly advised to conduct their own due diligence and register the candidate at their local police station as per Mumbai Police guidelines.
              </p>
            </div>

            <h2>4. Cancellations and Refunds</h2>
            <p>
              Once a candidate is selected and the placement fee is paid, the fee is non-refundable. If you decide not to proceed with the hired candidate, our replacement policy will apply.
            </p>

            <h2>5. Limitation of Liability</h2>
            <p>
              {BRAND.name} shall not be liable for any indirect, incidental, special, consequential, or punitive damages, or any loss of profits or revenues, whether incurred directly or indirectly, or any loss of data, use, goodwill, or other intangible losses, resulting from your use of our services or any conduct of the candidates.
            </p>

            <h2>6. Website Disclaimers & Illustrative Content</h2>
            <p>
              Please note the following regarding the content and claims presented on our website:
            </p>
            <ul>
              <li><strong>Illustrative Assets:</strong> The images, photographs, and user review names/avatars used on this website are for representational and reference purposes only. They may not depict actual staff members or clients and are used to illustrate the types of services we offer.</li>
              <li><strong>Service Timelines:</strong> Our stated timelines (e.g., "Profiles on WhatsApp in 30 minutes", "Replacement within 24 hours") are target service standards. Actual response times may vary due to human delays, technical glitches, or candidate availability.</li>
              <li><strong>Service Statistics & Claims:</strong> Claims such as "10000+ happy families" and other figures mentioned in our About section represent our projected business capacity, network scale, or historical industry experience, and are used for illustrative purposes.</li>
              <li><strong>Medical Screenings:</strong> Mentions of "Medically Screened" staff apply exclusively to specific premium service tiers or upon explicit request, as comprehensive medical testing for every candidate is not standard due to time and cost constraints.</li>
            </ul>

            <h2>7. Governing Law</h2>
            <p>
              These Terms shall be governed by and construed in accordance with the laws of India. Any disputes arising under or in connection with these Terms shall be subject to the exclusive jurisdiction of the courts located in Mumbai, Maharashtra.
            </p>

            <h2>8. Contact Information</h2>
            <p>If you have any questions about these Terms, please contact us at:</p>
            <p>
              Email: <a href={`mailto:${BRAND.email}`}>{BRAND.email}</a><br />
              Phone: {BRAND.phone}
            </p>

          </div>
        </div>
      </section>

      <Footer />
    </div>
  );
};

export default TermsOfService;
